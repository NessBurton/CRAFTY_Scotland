...
dirResults <- paste0(wd,"output/Thesis/")
#dirMetrics <- paste0(wd,"CRAFTY_Scotland/vision_metrics")

dfVision <-
  list.files(path = paste0(dirResults,"Green_Gold/"),
             pattern = "*.csv", 
             full.names = T) %>% 
...
ggplot(dfMetrics, aes(year_id,wood_cover, colour=vision))+
  geom_line()+
  theme_bw()
ggplot(dfMetrics, aes(year_id,wood_cover2, colour=vision))+
  geom_line()+
  theme_bw()
